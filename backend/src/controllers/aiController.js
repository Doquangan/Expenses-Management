...
// ========== OCR: Quét hoá đơn bằng Gemini Vision ==========
exports.scanInvoice = async (req, res) => {
  try {
    ...
    // Lấy danh sách categories của user để AI chọn đúng
    const categories = await Category.find({ $or: [{ user: null }, { user: userId }] });
    ...
    const prompt = `Bạn là trợ lý AI chuyên phân tích hình ảnh chi tiêu. Hãy xem bức ảnh này và trích xuất thông tin giao dịch.

Ảnh có thể là:
1. Hoá đơn / Bill mua hàng → Trích xuất số tiền tổng, tên cửa hàng, ngày giao dịch.
2. Ảnh chụp món đồ/đồ ăn có ghi chú số tiền trên ảnh (VD: "-20k", "50.000") → Nhận diện vật thể trong ảnh + đọc số tiền.
3. Ảnh chụp bất kỳ kèm text viết tay/đánh máy → Cố gắng hiểu ngữ cảnh.

DANH MỤC HIỆN CÓ: [${categoryList.join(', ')}]

QUY TẮC PHÂN LOẠI DANH MỤC:
- Hãy ưu tiên chọn 1 danh mục phù hợp nhất từ DANH MỤC HIỆN CÓ.
- Nếu KHÔNG CÓ danh mục nào thực sự phù hợp (độ khớp < 80%), hãy đề xuất 1 tên danh mục mới ngắn gọn (1-3 từ) và đặt "isNewCategory": true.

Trả về CHÍNH XÁC 1 đối tượng JSON (không có markdown block, không có backticks):
{
  "amount": <số tiền dương, kiểu number, đơn vị VNĐ>,
  "description": "<mô tả ngắn gọn>",
  "category": "<tên danh mục cũ hoặc tên mới đề xuất>",
  "isNewCategory": <true nếu đề xuất mới, false nếu dùng danh mục cũ>,
  "date": "<YYYY-MM-DD>",
  "type": "<'expense' hoặc 'income'>"
    ...
    // Trích xuất JSON từ response (xử lý trường hợp AI trả kèm markdown block)
    let jsonStr = rawText;
    ...
    res.status(500).json({ error: 'AI OCR failed', detail: err.message });
  }
};
